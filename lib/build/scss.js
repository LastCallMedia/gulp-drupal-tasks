...
module.exports = function (config) {
  ...
  function buildSass() {
    if (config.src.length > 0) {
      ...
      var autoprefixer = require('gulp-autoprefixer').default || require('gulp-autoprefixer');
      var csso = require('gulp-csso');
      var concat = require('gulp-concat');
      // For tilde imports (~package/path), add node_modules to loadPaths
      config.sassOptions.loadPaths = config.sassOptions.loadPaths || [];
      if (!config.sassOptions.loadPaths.includes('node_modules')) {
        config.sassOptions.loadPaths.push('node_modules');
      }
